...
class MitigationEngine : public grpc_core::RefCounted<MitigationEngine> {
 public:
  enum class Action { kCloseConnection };
...
// Specialization of ArenaContextType for MitigationEngine.
template <>
struct ArenaContextType<MitigationEngine> {
  static void Destroy(MitigationEngine* p) { p->Unref(); }
};
